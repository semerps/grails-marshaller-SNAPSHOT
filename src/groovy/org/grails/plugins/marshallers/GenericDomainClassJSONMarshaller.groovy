...
class GenericDomainClassJSONMarshaller implements ObjectMarshaller<JSON> {
    ...
    void marshalObject(Object v, JSON json) throws ConverterException {
        ...
        def logicalName = GrailsNameUtils.getLogicalPropertyName(ConverterUtil.trimProxySuffix(clazz.getName()), "Domain")
        def requestedConfig = RequestedFieldsConfigurationHolder.get(logicalName)
        ...
        for (GrailsDomainClassProperty property : properties) {
            if (!mc.ignore?.contains(property.getName())) {
                if (!requestedConfig || requestedConfig?.contains(property.getName())) {
        ...
        mc.virtual?.each { prop, callable ->
            if (!requestedConfig || requestedConfig?.contains(prop)) {
                writer.key(prop)
